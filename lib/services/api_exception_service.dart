...
class ClientConfirmationRequiredException extends ApiExceptionService {
  ClientConfirmationRequiredException({required HttpResponseMessageModel? responseMessage}) {
...
abstract class ApiExceptionMapper {
  ...
  static void controlException({Object? error, required BuildContext context}) {
    ...
    if (error is ApiExceptionService) {
      if (error is ConnectionException) {
        ...
        _showErrorAlert(errorMessage: "Connection Error Exception occurred!!", context: context);
        throw Future.error("Error Info", StackTrace.fromString("StackTrace Error message"));

      } else if (error is ClientErrorException) {

        _showErrorAlert(errorMessage: "Client Error Exception occurred!!", context: context);
        throw Future.error("Error Info", StackTrace.fromString("StackTrace Error message"));

      } else if (error is ServerErrorException) {

        _showErrorAlert(errorMessage: "Sever Error Exception occurred!!", context: context);
        throw Future.error("Error Info", StackTrace.fromString("StackTrace Error message"));

      } else {

        _showErrorAlert(errorMessage: "Unknown Error occurred!!", context: context);
        throw Future.error("Error Info", StackTrace.fromString("StackTrace Error message"));
  ...
  static void _showErrorAlert({required String errorMessage, required BuildContext context}) {
