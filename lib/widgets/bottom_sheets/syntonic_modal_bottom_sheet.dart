...
abstract class SyntonicModalBottomSheet<
  ...
  //       final double bottomHeight = renderBoxBottom.size.height;
  //       final bottomInset = MediaQuery.of(context).viewInsets.bottom;
  //       final _extent = (bottomHeight + childHeight + bottomInset + 34) / (constraints.maxHeight - kToolbarHeight * 2);
  //       print('キーボード処理中');
  //       print(bottomInset);
  ...
  //                       bottom: (constraints.maxHeight - kToolbarHeight * 2) * extentNotifier.value,
  //                       left: 0,
  //                       right: 0,
  ...
  //                           ? (MediaQuery.of(context).viewInsets.bottom + 250) * viewModel(ref).state.minExtent
  //                           : 0,
  //                       left: 0,
  //                       right: 0,
  //                       child: Container(
  ...
  //                                           ),
  //                                         ),
  //                                           SizedBox(key: bottomBarKey, child: bottomBar(context: context, ref: ref, extent: viewModel(ref).state.currentExtent),),
  //                                         ],),),
  //                                   ),
  //                                 ),
  //                               ),
...
class _ContentsWidgetState extends State<ContentsWidget> {
  ...
  late GlobalKey headerKey;
  late GlobalKey bottomBarKey;
  ...
  void initState() {
    ...
    // widget.viewModel.controller.addListener(() {
    //   if (widget.viewModel.state.currentExtent < 1) {
    //     return;
    //   }
    //   final double currentExtent = widget.viewModel.controller.pixels; // 現在のスクロール位置
    //   final double delta = currentExtent - _previousExtent; // 前回との差分
    //
    //   if (delta > 300) { // 下方向に100以上移動したら
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final double childHeight =
          (childKey.currentContext!.findRenderObject() as RenderBox)
              ...
              .height;
      height = childHeight;
      // final double headereight = (headerKey.currentContext!.findRenderObject() as RenderBox).size.height;
      // final double bottomHeight = (bottomBarKey.currentContext!.findRenderObject() as RenderBox).size.height;
      ...
      print(PlatformDispatcher.instance.views.first.viewPadding.top);
      // widget.viewModel.updateMinExtent(_extent);
      print(_extent);
  ...
  Widget build(BuildContext context) {
    double adjustedMinChildSize = (15 +
            kToolbarHeight +
            MediaQuery.of(
                    NavigationService().navigatorKey.currentState!.context)
                .viewPadding
                .top) /
        widget.constraints.maxHeight;
    // double adjustedMinChildSize = MediaQuery.of(context).viewInsets.bottom /
    //     (widget.constraints.maxHeight - kToolbarHeight);
    ...
    //     (widget.constraints.maxHeight -
    //         (PlatformDispatcher.instance.views.first.viewPadding.top -
    //             kToolbarHeight));
    // double actualHeight = _minExtent *
    //     (widget.constraints.maxHeight -
    //         (kToolbarHeight));
            ...
            child: Container(
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      // Padding(padding: EdgeInsets.only(bottom: height), child: DraggableScrollableSheet(builder: (BuildContext context, ScrollController controller) {
                      //   return AnimatedBottomSheet(additionalSheetChild: widget.additionalSheetChild);
                      // }),),
                      DraggableScrollableSheet(
                        initialChildSize: _minExtent,
